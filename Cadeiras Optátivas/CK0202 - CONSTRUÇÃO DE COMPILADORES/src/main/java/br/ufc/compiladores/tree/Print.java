...
import br.ufc.compiladores.temp.DefaultMap;
...
public class Print {
   ...
   void prStm(Stm s, int d) {
      ...
      System.out.println("Tipo de Stm não tratado: " + s.getClass());
   ...
   void prExp(Exp1 e, int d) {
      if (e == null) {
         indent(d);
         sayln("NULL EXPRESSION");
         return;
     }
      ...
      System.out.println("Tipo de expressão não tratado: " + e.getClass().getName());
      throw new Error("Print.prExp");
   }
   }

   public void prStm(Stm s) {prStm(s,0); say("\n");}
   public void prExp(Exp1 e) {prExp(e,0); say("\n");}
